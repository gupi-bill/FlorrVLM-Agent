...
0. **方向先行**：执行任何阶段前先读 `devplan/DIRECTION.md`（这是什么 / 我该往哪推 / 用户该往哪走）。路线判断以它为准，README 的措辞不作数。
1. 每周期从第一个未完成阶段开始，**做完一个阶段后若本轮时间预算（25 分钟）仍有剩余，立刻继续下一阶段**，直到预算用完或遇阻断；不要只做一个阶段就停。
2. **S14 完成后自动进入第二阶段工作表 `devplan/PLAN_PHASE2.md`（S15~S26）**，规则与本表一致。
2b. 开始先读 `devplan/PROGRESS.md` 定位当前阶段；结束必须更新该文件（阶段状态、产物清单、测试结果、遗留问题、下一步）。
